...
ID3D12Resource* CreateTextureResource(ID3D12Device* device,
	...
	resourceDesc.DepthOrArraySize =
		UINT16(metadata.arraySize);        // 奥行 or 配置Textureの配列数
...
ID3D12Resource* CreateDepthStencilTextureResource(ID3D12Device* device,
	...
	depthClearValue.Format =
		DXGI_FORMAT_D24_UNORM_S8_UINT; // フォーマット。Resourceの合わせる
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			transformationMatrixDataSphere->WVP = viewMatrixSphere;

			Matrix4x4 projectionMatrixSphere = MakePerspectiveFovMatrix(
				0.45f, float(kClientWidth) / float(kClientHeight), 0.1f, 100.0f);

			transformationMatrixDataSphere->WVP = projectionMatrixSphere;
			...
			// マテリアルCBufferの場所を設定
			commandList->SetGraphicsRootConstantBufferView(
				3, materialResourcedirectionalLight->GetGPUVirtualAddress());
